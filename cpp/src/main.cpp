...
double hit_sphere(const point3 &center, double radius, const ray &r) {
  ...
  if (discriminant < 0) {
    ...
    return (-half_b - std::sqrt(discriminant)) / (2.0 * a);
  }
}
...
hittable_list final_scene() {
  ...
  auto emat =
      make_shared<lambertian>(make_shared<image_texture>("assets/earthmap.jpg"));
...
hittable_list simple_light() {
  ...
  objects.add(make_shared<sphere>(point3(0, 8, 0), 2,
...
hittable_list earth() {
  auto earth_texture = make_shared<image_texture>("assets/jupiter.jpg");
